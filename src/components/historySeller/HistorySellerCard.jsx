/**
 * Tarjeta del seller: thinking, active, rejected, finalized, transaction.
 */
...
import HistorySellerActions from './HistorySellerActions';

export default function HistorySellerCard({ entry, ctx, actions, index: _index }) {
  const {
    formatPriceInt,
    formatAddress,
    formatCardDate,
    ...
    return (
      ...
      >
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          ...
          <button
            onClick={() => actions.dismissThinking(item)}
        ...
        <HistorySellerActions
          onAccept={() => actions.acceptThinking(item)}
          onDismiss={() => actions.dismissThinking(item)}
          onReject={() => actions.rejectThinking(item)}
        />
      </motion.div>
    );
  }

  if (entry.__type === 'active') {
    ...
    if (ctx.hiddenKeys?.has(cardKey)) return null;
...
export function HistorySellerFinalCard({ item, ctx, index }) {
  const {
    formatPriceInt,
    formatAddress,
    formatCardDate,
