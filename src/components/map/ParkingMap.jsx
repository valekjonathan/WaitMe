...
// Cache de iconos (evita recrear SVG/Icon en cada render y reduce tirones)
const __carIconCache = new Map();

function getCarIconCached(color, price, vehicleType = 'car') {
  const key = `${color || ''}|${price ?? ''}|${vehicleType || 'car'}`;
  const cached = __carIconCache.get(key);
  if (cached) return cached;
  const icon = createCarIcon(color, price, vehicleType);
  __carIconCache.set(key, icon);
  return icon;
}
...
// Singleton (no recrear cada render)
const USER_LOCATION_ICON = createUserLocationIcon();

let BUYER_LOCATION_ICON = null;
function getBuyerLocationIcon() {
  if (BUYER_LOCATION_ICON) return BUYER_LOCATION_ICON;
  BUYER_LOCATION_ICON = L.divIcon({
...
export default function ParkingMap({
  ...
  // Evita crasheos si llegan alertas incompletas (lat/lng undefined)
  ...
  const safeBuyerLocations = useMemo(() => {
    const list = Array.isArray(buyerLocations) ? buyerLocations : [];
    return list.filter((loc) => {
      const lat = Number(loc?.latitude);
      const lng = Number(loc?.longitude);
      return loc?.id != null && Number.isFinite(lat) && Number.isFinite(lng);
    });
  }, [buyerLocations]);
    ...
    <div className={`relative ${className}`}>
      ...
      >
        ...
        {normalizedUserLocation && (
          <Marker position={normalizedUserLocation} icon={USER_LOCATION_ICON} draggable={isSelecting}></Marker>
        )}

        {/* Buyer locations (usuarios en camino - tracking en tiempo real) */}
        {safeBuyerLocations.map((loc) => (
          <Marker
            key={loc.id}
            position={[loc.latitude, loc.longitude]}
            icon={getBuyerLocationIcon()}
            zIndexOffset={1500}
          >
        ...
        {isSelecting && selectedPosition && selectedPosition.lat !== normalizedUserLocation?.[0] && (
          <Marker position={selectedPosition} icon={USER_LOCATION_ICON}></Marker>
        )}
        ...
        {safeAlerts.map((alert) => (
          <Marker
            ...
            icon={getCarIconCached(alert.car_color, alert.price, alert.vehicle_type)}
            eventHandlers={{
