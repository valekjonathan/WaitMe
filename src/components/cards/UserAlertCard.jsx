  ...
  return (
    <div className="bg-gray-900 border border-gray-700 rounded-2xl p-4 flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-2">
        {/* INFO USUARIO → MISMO ANCHO QUE FOTO */}
        <div className="w-14 text-center text-xs bg-purple-600/20 text-purple-400 py-0.5 rounded-md">
          Info usuario
        </div>

        <div className="flex items-center gap-2 text-xs text-gray-300">
          <span>{alert?.distance || '0.1km'}</span>
          <span className="bg-purple-600 px-2 py-0.5 rounded-md text-white">
            {alert?.price || '5.00€'}
          </span>
        </div>
      </div>
      ...
      <div className="flex gap-2 mt-auto">
        <Button variant="outline" size="icon" className="border-gray-700" onClick={onChat}>
          <MessageCircle size={18} />
        </Button>

        <Button variant="outline" size="icon" className="border-gray-700" onClick={onCall}>
          <Phone size={18} />
        </Button>
