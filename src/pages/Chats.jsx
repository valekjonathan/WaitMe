...
export default function Chats() {
  ...
  // Usuario autenticado (una sola llamada y cacheada)
  ...
  // Ubicación (solo para mostrar distancia; si falla, no bloquea la UI)
  useEffect(() => {
    if (!navigator.geolocation) return;
    navigator.geolocation.getCurrentPosition(
      (pos) => setUserLocation([pos.coords.latitude, pos.coords.longitude]),
      () => {},
      { enableHighAccuracy: false, timeout: 3000, maximumAge: 60 * 1000 }
    );
  }, []);

  // Conversaciones SOLO del usuario (2 queries ligeras)
  ...
  // IDs que necesitamos para resolver datos (solo los que aparecen en conversaciones)
  const ids = useMemo(() => {
  ...
  const filteredConversations = useMemo(() => {
    ...
    return conversations.filter((conv) => {
      ...
      const otherName = other?.display_name || other?.full_name || (isP1 ? conv.participant2_name : conv.participant1_name) || '';
