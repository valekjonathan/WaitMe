...
export default function History() {
  ...
  // ✅ ESTADO ENCENDIDO (Igual que el countdown)
  const StatusBoxPurple = ({ text }) => (
    <div className="w-full h-8 rounded-lg border-2 border-purple-400/70 bg-purple-600/25 flex items-center justify-center">
      <span className="text-sm font-mono font-extrabold text-purple-100">{text}</span>
    </div>
  );
  ...
  const MarcoContent = ({ photoUrl, name, carLabel, plate, carColor, statusText, address, bright = false }) => {
    ...
    // Si es Cancelada o Expirada en Reservas, usamos StatusBoxPurple
    const isSpecialStatus = ['CANCELADA', 'EXPIRADA'].includes(statusText?.toUpperCase());
        ...
        <div className="border-t border-gray-800/80 my-2 pt-2 space-y-1.5">
          ...
          <div className="flex gap-2 mt-1">
            ...
            <div className="flex-1">
              {isSpecialStatus ? (
                <StatusBoxPurple text={statusText} />
              ) : (
                <div className={`w-full h-8 rounded-lg border-2 flex items-center justify-center font-mono font-bold text-sm ${bright ? 'border-purple-500/40 bg-purple-500/10 text-purple-200' : 'border-gray-700 bg-gray-800/40 text-gray-500'}`}>
                  {statusText}
                </div>
              )}
            </div>
      ...
      <main className="pt-14 pb-20 px-4">
        <Tabs defaultValue="alerts" className="w-full">
          ...
          <TabsContent value="alerts" className={`mt-0 space-y-1.5 ${noScrollBar} overflow-y-auto max-h-[75vh]`}>
            ...
            {/* ✅ EJEMPLO INVENTADO: Alerta cancelada por ti */}
            <div className="bg-gray-900/30 rounded-xl p-3 border border-gray-700/80 mb-2">
              <div className="flex justify-between items-center mb-2">
                <Badge className="bg-red-500/10 text-red-400 border-red-500/20 uppercase">Cancelada</Badge>
                <span className="text-[10px] text-gray-500">Hoy - 12:45</span>
                <MoneyChip amountText="2.50€" />
              </div>
              <div className="w-full h-8 rounded-lg border-2 border-purple-400/70 bg-purple-600/25 flex items-center justify-center text-purple-100 font-mono font-extrabold text-sm">
                CANCELADA POR TI
              </div>
            </div>

            {finalizedAlerts.map(alert => (
              <div key={alert.id} className="bg-gray-900/30 rounded-xl p-3 border border-gray-700/80 mb-2">
                <div className="flex justify-between items-center mb-2">
