...
export default function History() {
  ...
  // 1️⃣ Fuente de verdad si existe
  const explicitMs = toMs(alert?.expires_at);
  if (explicitMs) return explicitMs;

  // 2️⃣ Fallback correcto: minutos seleccionados DESDE publicación real
  const mins = Number(alert?.available_in_minutes);
  if (!Number.isFinite(mins)) return null;

  // 🔒 Congelamos el inicio del contador una sola vez
  if (!alert.__waitmeStartTs) {
    alert.__waitmeStartTs = Date.now();
  }

  return alert.__waitmeStartTs + mins * 60 * 1000;
};
