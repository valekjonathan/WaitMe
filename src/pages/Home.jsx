...
export default function Home() {
  ...
  useEffect(() => {
    const goLogo = () => {
      // ✅ Vuelve al logo y resetea estados internos del Home (sin tocar nada visual)
      setMode(null);
      try { setShowFilters(false); } catch {}
      try { setSelectedAlert(null); } catch {}
      try { setConfirmDialog({ open: false, alert: null }); } catch {}
      try { setSearchQuery(''); } catch {}
      try { setSelectedPosition(null); } catch {}
      // Refresca datos por si estaban en estados intermedios
 ...
 // null | 'search' | 'create'
  ...
  // 🔥 Pre-carga del logo para que no “se rompa” en Base44/iOS si tarda en descargar
  useEffect(() => {
    try {
      const img = new Image();
      img.src = logoSrc;
